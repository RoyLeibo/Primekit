...
SubscriptionInfo _activeInfo(String productId) => SubscriptionInfo(
      productId: productId,
      status: SubscriptionStatus.active,
    );

SubscriptionInfo _expiredInfo(String productId) => SubscriptionInfo(
      productId: productId,
...
void main() {
  ...
  group('configure() / instance', () {
    test('throws ConfigurationException before configure()', () {
      expect(() => EntitlementChecker.instance, throwsA(isA<ConfigurationException>()));
    });
    ...
    test('isConfigured returns true after configure()', () {
      EntitlementChecker.configure(
        ...
        featureToProductMap: {'export_pdf': ['pro']},
      );
    ...
    test('calling configure() again overwrites previous config', () {
      EntitlementChecker.configure(
        subscriptionManager: manager,
        featureToProductMap: {'old_feature': ['old_product']},
      );
      EntitlementChecker.configure(
        subscriptionManager: manager,
        featureToProductMap: {'new_feature': ['new_product']},
      );
      final products = EntitlementChecker.instance
          .productsForFeature('new_feature');
  ...
  group('check()', () {
    ...
    test('returns EntitlementAllowed when product is active', () async {
      ...
      final result =
          await EntitlementChecker.instance.check('export_pdf');
    ...
    test('returns EntitlementAllowed for second product in list', () async {
      ...
      when(() => mockDataSource.fetchSubscription('pro_yearly'))
    ...
    test('returns EntitlementDenied for unknown feature', () async {
      final result =
          await EntitlementChecker.instance.check('nonexistent_feature');
      expect(result, isA<EntitlementDenied>());
    });
  ...
  group('canAccess()', () {
    setUp(() {
      EntitlementChecker.configure(
        subscriptionManager: manager,
        featureToProductMap: {'dark_theme': ['pro_monthly']},
      );
    });
    ...
    test('returns false for unknown feature', () async {
      expect(
        await EntitlementChecker.instance.canAccess('unknown'),
        isFalse,
      );
    });
  });
  ...
  group('accessibleFeatures()', () {
    ...
    test('returns only features with active subscriptions', () async {
      when(() => mockDataSource.fetchSubscription('pro_monthly'))
          .thenAnswer((_) async => _activeInfo('pro_monthly'));
      when(() => mockDataSource.fetchSubscription('enterprise'))
          .thenAnswer((_) async => null);

      final features =
          await EntitlementChecker.instance.accessibleFeatures();
  ...
  group('productsForFeature()', () {
    ...
    test('returns the product ids for a known feature', () {
      final products =
          EntitlementChecker.instance.productsForFeature('export_pdf');
      expect(products, containsAll(['pro_monthly', 'pro_yearly']));
    });

    test('returns empty list for unknown feature', () {
      final products =
          EntitlementChecker.instance.productsForFeature('unknown');
