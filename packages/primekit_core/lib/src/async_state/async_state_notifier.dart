...
class AsyncStateNotifier<T> extends ChangeNotifier {
  ...
  ///
  /// Each call to [run] or [refresh] captures the current id before awaiting.
  /// If [_operationId] has advanced by the time the future resolves, the result
  /// is silently dropped to prevent a slower earlier operation from overwriting
  /// a newer one.
  int _operationId = 0;
