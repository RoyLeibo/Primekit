...
final class PkStringSchema extends PkSchema<String> {
  ...
  /// Validates that the value matches [pattern] (custom regex).
  PkStringSchema matches(RegExp pattern, {String? message}) => _copyWith(
    extraRules: [
      _StringRule(
        test: (v) => pattern.hasMatch(v),
        message: message ?? 'Invalid format',
      ),
    ],
  );

  /// Validates that the value has no leading or trailing whitespace.
  PkStringSchema noWhitespace({String? message}) => _copyWith(
    extraRules: [
      _StringRule(
        test: (v) => v == v.trim(),
        message: message ?? 'Must not have leading or trailing spaces',
      ),
    ],
  );

  /// Validates that the value contains only letters and digits.
  PkStringSchema alphanumeric({String? message}) => _copyWith(
    extraRules: [
      _StringRule(
        test: (v) => _alphanumericRegex.hasMatch(v),
  ...
  /// final confirm = PkSchema.string().equalTo(password).required();
  /// ```
  ///
  ...
  PkStringSchema equalTo(PkStringSchema other, {String? message}) =>
      PkStringSchema._(
        required: _required,
        rules: _rules,
